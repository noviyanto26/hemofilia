# 19_rekap_hemofilia.py — Postgres/Supabase rework (fix Excel tz-aware datetimes)
import io
import re
...
# ======================== NEW: sanitasi datetime agar timezone-naive ========================
...
def make_excel_safe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Excel (xlsxwriter) tidak menerima datetime yang timezone-aware (mis. timestamptz).
    Ubah semua kolom datetime menjadi timezone-naive sebelum to_excel.
    """
    ...
    for c in d.columns:
        s = d[c]
        try:
            # Kolom sudah datetime bertimezone
            if is_datetime64tz_dtype(s):
                d[c] = s.dt.tz_localize(None)
                continue
            # Kolom object: coba parse ke datetime(utc) lalu lepas tz
            ...
            # Abaikan error parsing kolom individual agar ekspor tetap jalan
            pass
    return d
...
with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
    for t in tables_selected:
        ...
        # >>> Perbaikan utama: pastikan df bebas timezone sebelum ditulis ke Excel
        df_xl = make_excel_safe(df if not df.empty else pd.DataFrame())

        df_xl.to_excel(writer, index=False, sheet_name=safe_name)
        autosize_and_print_setup(writer, safe_name, df_xl)
